package com.hnit.video.ucenter.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.hnit.video.ucenter.mapper.MemberMapper;
import com.hnit.video.ucenter.service.MemberService;
import com.hnit.video.common.base.exception.MyException;
import com.hnit.video.common.utils.JwtUtil;
import com.hnit.video.common.utils.MD5Util;
import com.hnit.video.model.entity.ucenter.Member;
import com.hnit.video.model.vo.ucenter.admin.MemberQueryCondition;
import com.hnit.video.model.vo.ucenter.user.LoginVO;
import com.hnit.video.model.vo.ucenter.user.PasswordVO;
import com.hnit.video.model.vo.ucenter.user.RegisterVO;
...
public class MemberServiceImpl extends ServiceImpl<MemberMapper, Member> implements MemberService {
    ...
    public String userLogin(LoginVO loginVO) {
        ...
        if (member == null) {
            throw new MyException("用户不存在,请注册");
        }
        // 用户名和密码都正确则应该判断用户是否被禁用
        if (!member.getIsEnable()) {
            throw new MyException("用户账号被禁用,请申诉");
        }
        if (!password.equals(member.getPassword())) {
            throw new MyException("密码错误");
        }
        // 登录成功,返回toke
        //将登陆成功的用户存入到statistics_daily表中
    ...
    public void userRegister(RegisterVO registerVO) {
        ...
        if (mobileData != null) throw new MyException("用户已经注册,请登录");
    ...
    public void updatePassword(PasswordVO passwordVO, Member userMember) {
        ...
        if (!member.getId().equals(userMember.getId())) {
            throw new MyException("修改用户和当前登录用户不同,请重试");
        }
        // 判断验证码
        // 获取redis验证码
